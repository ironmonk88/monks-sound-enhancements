...
export class ActorSounds {
        ...
        if (!(setting("actor-sounds") === "none" || setting("actor-sounds") === 'false')) {
            ...
            sheetNames.forEach((sheetName) => {
                Hooks.on("render" + sheetName, (app, html, data) => {
                    ...
                    if (!soundEffect) {
                        soundEffect = getProperty(app.document, "flags.monks-little-details.sound-effect");
                        if (soundEffect)
    ...
    static async loadSoundEffect(token, options = {}, event) {
        ...
        if (!token.soundeffect && options.action != "stop") {
            ...
            let soundEffect = getProperty(actor, "flags.monks-sound-enhancements.sound-effect");
            if (!soundEffect) {
                soundEffect = getProperty(actor, "flags.monks-little-details.sound-effect");
                if (soundEffect)
